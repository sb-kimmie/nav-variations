...
function NavLink({ lk, onClose, className, style, children }) {
  if (lk.disabled) {
    return (
      <span className={className} style={{ color: '#9aa5b4', cursor: 'default', ...style }}>
        {children ?? lk.label}
      </span>
    );
  }
  return (
    <Link href={lk.href} onClick={onClose} className={className} style={style}>
      {children ?? lk.label}
    </Link>
  );
}
