...
/* ─────────────────────────────────────────────
   ...
   Panel bg:        #004A8A
   Left card:       #01346a
   Section heading: #01346a band + white left border + white text
   Sub-links:       white/80 text, hover white + white/10 bg
...
function MegaPanel({ item, isOpen, onMouseEnter, onMouseLeave, onClose }) {
  return (
    <div
      ...
      `}
    >
      {/* Panel: #004A8A bg, white bottom border accent */}
      <div className="bg-[#004a8a] border-t border-white/10 border-b-4 border-b-white/30 shadow-[0_24px_64px_rgba(0,0,0,0.3),0_4px_16px_rgba(0,0,0,0.15)]">
        <div className="max-w-[1280px] mx-auto px-8 py-10 grid grid-cols-[240px_1fr] gap-x-12 items-start">
          ...
          <div className="bg-[#01346a] rounded-lg p-6 flex flex-col gap-4 self-stretch">
            <h2 className="font-playfair text-2xl font-bold text-white leading-tight m-0">
              {item.label}
            </h2>
            {item.description && (
              <p className="text-[13.5px] leading-[1.7] text-white/70 m-0 flex-1">
                {item.description}
              </p>
            )}
            <Link
          ...
          <div className="grid grid-cols-[repeat(auto-fill,minmax(190px,1fr))] gap-x-2">
            {item.sections.map((sec) => (
              <div key={sec.heading} className="flex flex-col">
                ...
                {/* HEADING — #01346a band, white left border, white bold uppercase */}
                <Link
                  href={sec.href}
                  onClick={onClose}
                  className="
                ...
                {/* SUB-LINKS — white/80 text, clearly subordinate */}
                <ul
...
function MobSection({ sec }) {
  ...
  return (
    <div className="border-b border-white/[0.04]">
      {/* Section heading — navy block with white left stripe */}
      <button
        onClick={() => setOpen((o) => !o)}
        aria-expanded={open}
